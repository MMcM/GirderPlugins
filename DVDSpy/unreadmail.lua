...
-- Use Girder's registry object.
function unreadmail.FromRegistry(self, allAccounts)
  local reg = win.CreateRegistry("HKCU", self.UnreadMailRoot)
  local keys = reg:ListKeys()

  local function getval(key)
    local reg = win.CreateRegistry("HKCU", self.UnreadMailRoot .. "\\" .. key)
    local result = { UnRead = reg:Read("MessageCount") }
...
-- The following works even outside Girder.
unreadmail.HKCU = 2147483649
...
function unreadmail.FromOutlook(self, includeJunk, includeOtherStores)
  -- GetObject or nil
